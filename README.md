...
👉 **[Смотреть видеообзор на Rutube](https://rutube.ru/video/private/f51fbe009ef74da5cb3fcbe7f8f2a509/?p=ygwTU6mdq_iwAXTVAFSDOg)**
...
Для корректной работы скрипта в вашей папке `moonloader/lib` должны быть установлены следующие компоненты:
...
* **SAMPFUNCS 5.4.1**
* **Библиотеки:**
    * `SAMP.Lua` (папка `samp` внутри `lib`) - без неё скрипт не сможет читать чат и диалоги.
    * `encoding` - необходима для работы русского текста и корректной записи данных в таблицы.
    * `vkeys` - стандартная библиотека для работы с клавишами (F5, Ctrl+R, I).
...
* 🔄 **P2P Sync:** Мгновенная внутриигровая синхронизация между всеми пользователями скрипта. Зачет, поставленный одним инструктором, сразу отображается у всех.
...
* ☁️ **Cloud Integration:** * **Google Forms:** При сдаче теории отметка мгновенно появляется в таблице и в игре (требуется доступ автора к форме).
    * **Auto-Promotion Transfer:** После повышения кадета данные переносятся в основную таблицу LVPD (лист CPD). (требуется доступ автора к таблице).

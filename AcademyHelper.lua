script_name("AcademyHelper_Stable_v2.2")
...
local needSync = false -- Флаг для безопасного обновления
...
function updateFromBase()
    ...
    lua_thread.create(function()
        ...
        print("[AH] Синхронизация с облаком...")
...
function updateCadetInBase(name, col)
    ...
    sampAddChatMessage("{0633E5}[AH] {FFFFFF}Обновляем " .. name:gsub("_", " ") .. "...", -1)
    ...
    downloadUrlToFile(fullUrl, tempFile, function(id, status, p1, p2)
        ...
        if status == 6 then
            ...
            print("[AH] Запись в таблицу подтверждена.")

            needSync = true -- Ставим флаг, main обновит базу сам
...
function main()
    ...
    while true do
        ...
        -- Безопасное обновление после записи

        if needSync then

            wait(2000) -- Пауза в главном потоке разрешена

            updateFromBase()

            sampAddChatMessage("{00FF00}[AH] {FFFFFF}Данные синхронизированы!", -1)
...
function sampev.onSendDialogResponse(id, btn, lst, inp)
    if id == 9910 and btn == 1 then
        ...
        if selectedCadet then
            lua_thread.create(function()
                wait(50)
                -- ДОБАВИЛИ ПУНКТ "Сбросить данные"
    ...
    if id == 9911 then
        if btn == 1 and selectedCadet then
            -- Теперь здесь 4 варианта действий
            local actions = {"lecture", "theory", "practice", "reset"}
            local currentAction = actions[lst + 1]
            
            -- Если выбран сброс, сразу визуально "красим" в красный в локальной базе для мгновенного эффекта
...
function sampev.onServerMessage(clr, txt)
    ...
    if isUpdating then
        ...
        if c:find("Кадет") or c:find("Cadet") then
        ...
        if c:find("Члены организации") or c:match("ID:%s*%d+") or c:find("Всего%:") then

            if c:find("Всего%:") then isUpdating = false end
